...
namespace CZJ.Common.Core
{
    ...
    public class FileHelper
    {
        ...
        {
            path = path.TrimStart('~', '/', '\\').Replace("/", DirectorySeparatorChar).Replace("\\", DirectorySeparatorChar);
            return IsAbsolute(path) ? path.Replace("@", "") : Path.Combine(_ContentRootPath, path.Replace("@", ""));
        }
